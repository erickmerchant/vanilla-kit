import {watch, effect, create, adopt, svg_namespace} from "../lib.js";
...
export default function todoApp(target) {
	...
	let listOl = create("ol")
		.classes("list")
		.map(
			state.list,
			itemView,
			(item) => state.showDone || !item.isDone || item.isLeaving
		);
